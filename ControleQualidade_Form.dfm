object FrmCtrlQualidade: TFrmCtrlQualidade
  Left = 14
  Top = 8
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Controle de Qualidade'
  ClientHeight = 517
  ClientWidth = 633
  Color = 15396334
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlClient: TPanel
    Left = 0
    Top = 0
    Width = 633
    Height = 517
    Align = alClient
    BevelInner = bvRaised
    BevelOuter = bvLowered
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    OnResize = pnlClientResize
    object Bevel1: TBevel
      Left = 2
      Top = 483
      Width = 629
      Height = 2
      Align = alBottom
      Shape = bsTopLine
    end
    object b2: TBevel
      Left = 8
      Top = 35
      Width = 184
      Height = 2
      Shape = bsTopLine
    end
    object Label1: TcxLabel
      Left = 86
      Top = 56
      Caption = 'C'#243'digo'
      ParentFont = False
      Transparent = True
    end
    object Label2: TcxLabel
      Left = 63
      Top = 81
      Caption = 'Virabrequim'
      ParentFont = False
      Transparent = True
    end
    object LblTitulo: TcxLabel
      Left = 6
      Top = 8
      Caption = 'Controle de Qualidade'
      ParentFont = False
      Transparent = True
    end
    object Label3: TcxLabel
      Left = 49
      Top = 106
      Caption = 'Medida Mancal'
      ParentFont = False
      Transparent = True
    end
    object Label4: TcxLabel
      Left = 60
      Top = 131
      Caption = 'Medida Biela'
      ParentFont = False
      Transparent = True
    end
    object Label5: TcxLabel
      Left = 8
      Top = 156
      Caption = 'Folga Axial Virabrequim'
      ParentFont = False
      Transparent = True
    end
    object Label6: TcxLabel
      Left = 73
      Top = 225
      Caption = 'Cabe'#231'ote'
      ParentFont = False
      Transparent = True
    end
    object Label7: TcxLabel
      Left = 41
      Top = 250
      Caption = 'Altura Cabe'#231'ote'
      ParentFont = False
      Transparent = True
    end
    object Label8: TcxLabel
      Left = 27
      Top = 275
      Caption = 'Alt. Valv. Admiss'#227'o'
      ParentFont = False
      Transparent = True
    end
    object Label9: TcxLabel
      Left = 38
      Top = 300
      Caption = 'Alt. Valv. Escape'
      ParentFont = False
      Transparent = True
    end
    object Label10: TcxLabel
      Left = 94
      Top = 374
      Caption = 'Bloco'
      ParentFont = False
      Transparent = True
    end
    object Label11: TcxLabel
      Left = 55
      Top = 399
      Caption = 'Folga Cilindro'
      ParentFont = False
      Transparent = True
    end
    object Label12: TcxLabel
      Left = 61
      Top = 424
      Caption = 'Folga Radial'
      ParentFont = False
      Transparent = True
    end
    object Label13: TcxLabel
      Left = 417
      Top = 81
      Caption = 'Biela'
      ParentFont = False
      Transparent = True
    end
    object Label14: TcxLabel
      Left = 356
      Top = 106
      Caption = 'Folga Radial Biela'
      ParentFont = False
      Transparent = True
    end
    object Label15: TcxLabel
      Left = 375
      Top = 131
      Caption = 'Folga do Pino'
      ParentFont = False
      Transparent = True
    end
    object Label16: TcxLabel
      Left = 387
      Top = 156
      Caption = 'Folga Axial'
      ParentFont = False
      Transparent = True
    end
    object Label17: TcxLabel
      Left = 396
      Top = 225
      Caption = 'Ajustado'
      ParentFont = False
      Transparent = True
    end
    object Label18: TcxLabel
      Left = 369
      Top = 250
      Caption = 'Torque Mancal'
      ParentFont = False
      Transparent = True
    end
    object Label19: TcxLabel
      Left = 380
      Top = 275
      Caption = 'Torque Biela'
      ParentFont = False
      Transparent = True
    end
    object Label20: TcxLabel
      Left = 378
      Top = 300
      Caption = 'Altura Pist'#227'o'
      ParentFont = False
      Transparent = True
    end
    object Label21: TcxLabel
      Left = 329
      Top = 381
      Caption = 'Observa'#231#227'o'
      ParentFont = False
      Transparent = True
    end
    object Label22: TcxLabel
      Left = 60
      Top = 181
      Caption = 'Colaborador'
      ParentFont = False
      Transparent = True
    end
    object BtnMecanico: TcxButton
      Left = 283
      Top = 177
      Width = 25
      Height = 22
      Cursor = crHandPoint
      TabOrder = 57
      OnClick = BtnMecanicoClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBF}
    end
    object Label23: TcxLabel
      Left = 60
      Top = 325
      Caption = 'Colaborador'
      ParentFont = False
      Transparent = True
    end
    object BtnMontadorCabecote: TcxButton
      Left = 283
      Top = 321
      Width = 25
      Height = 22
      Cursor = crHandPoint
      TabOrder = 59
      OnClick = BtnMontadorCabecoteClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBF}
    end
    object Label24: TcxLabel
      Left = 381
      Top = 181
      Caption = 'Colaborador'
      ParentFont = False
      Transparent = True
    end
    object BtnMontadorBiela: TcxButton
      Left = 600
      Top = 177
      Width = 25
      Height = 22
      Cursor = crHandPoint
      TabOrder = 61
      OnClick = BtnMontadorBielaClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBF}
    end
    object Label25: TcxLabel
      Left = 380
      Top = 325
      Caption = 'Colaborador'
      ParentFont = False
      Transparent = True
    end
    object BtnMontadorAjuste: TcxButton
      Left = 603
      Top = 321
      Width = 25
      Height = 22
      Cursor = crHandPoint
      TabOrder = 63
      OnClick = BtnMontadorAjusteClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBF}
    end
    object Label26: TcxLabel
      Left = 60
      Top = 449
      Caption = 'Colaborador'
      ParentFont = False
      Transparent = True
    end
    object BtnMontadorBloco: TcxButton
      Left = 283
      Top = 445
      Width = 25
      Height = 22
      Cursor = crHandPoint
      TabOrder = 65
      OnClick = BtnMontadorBlocoClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBF}
    end
    object Label27: TcxLabel
      Left = 361
      Top = 357
      Caption = 'Montador Motor'
      ParentFont = False
      Transparent = True
    end
    object BtnMontador: TcxButton
      Left = 602
      Top = 353
      Width = 25
      Height = 22
      Cursor = crHandPoint
      TabOrder = 67
      OnClick = BtnMontadorClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBF}
    end
    object pnlBottom: TPanel
      Left = 2
      Top = 485
      Width = 629
      Height = 30
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 27
      object dxBarDockControl1: TdxBarDockControl
        Left = 0
        Top = 2
        Width = 629
        Height = 26
        Align = dalTop
        BarManager = BarManager
      end
      object Panel1: TPanel
        Left = 0
        Top = 0
        Width = 629
        Height = 2
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 1
      end
    end
    object EdInicial: TdxDBEdit
      Left = 126
      Top = 76
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 1
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'VIRABREQUIM'
      DataSource = DataSource
    end
    object DbCodigo: TdxDBEdit
      Left = 126
      Top = 51
      Width = 82
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 0
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'CODIGO'
      DataSource = DataSource
    end
    object dxDBEdit1: TdxDBEdit
      Left = 126
      Top = 101
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 2
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'MEDIDA_MANCAL'
      DataSource = DataSource
    end
    object dxDBEdit2: TdxDBEdit
      Left = 126
      Top = 126
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 3
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'MEDIDA_BIELA'
      DataSource = DataSource
    end
    object dxDBEdit3: TdxDBEdit
      Left = 126
      Top = 151
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 4
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'FOLGA_AXIAL_VIRABREQUIM'
      DataSource = DataSource
    end
    object dxDBEdit4: TdxDBEdit
      Left = 126
      Top = 220
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 6
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'CABECOTE'
      DataSource = DataSource
    end
    object dxDBEdit5: TdxDBEdit
      Left = 126
      Top = 245
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 7
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'ALTURA_CABECOTE'
      DataSource = DataSource
    end
    object dxDBEdit6: TdxDBEdit
      Left = 126
      Top = 270
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 8
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'ALTURA_VALV_ADMISSAO'
      DataSource = DataSource
    end
    object dxDBEdit7: TdxDBEdit
      Left = 126
      Top = 295
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 9
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'ALTURA_VALV_ESCAPE'
      DataSource = DataSource
    end
    object dxDBEdit8: TdxDBEdit
      Left = 126
      Top = 369
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 11
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'BLOCO'
      DataSource = DataSource
    end
    object dxDBEdit9: TdxDBEdit
      Left = 126
      Top = 394
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 12
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'FOLGA_CILINDRO'
      DataSource = DataSource
    end
    object dxDBEdit10: TdxDBEdit
      Left = 126
      Top = 419
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 13
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'FOLGA_RADIAL_BLOCO'
      DataSource = DataSource
    end
    object dxDBEdit11: TdxDBEdit
      Left = 446
      Top = 76
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 15
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'BIELA'
      DataSource = DataSource
    end
    object dxDBEdit12: TdxDBEdit
      Left = 446
      Top = 101
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 16
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'FOLGA_RADIAL_BIELA'
      DataSource = DataSource
    end
    object dxDBEdit13: TdxDBEdit
      Left = 446
      Top = 126
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 17
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'FOLGA_PINO'
      DataSource = DataSource
    end
    object dxDBEdit14: TdxDBEdit
      Left = 446
      Top = 151
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 18
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'FOLGA_AXIAL_BIELA'
      DataSource = DataSource
    end
    object dxDBEdit15: TdxDBEdit
      Left = 446
      Top = 220
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 20
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'AJUSTADO'
      DataSource = DataSource
    end
    object dxDBEdit16: TdxDBEdit
      Left = 446
      Top = 245
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 21
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'TORQUE_MANCAL'
      DataSource = DataSource
    end
    object dxDBEdit17: TdxDBEdit
      Left = 446
      Top = 270
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 22
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'TORQUE_BIELA'
      DataSource = DataSource
    end
    object dxDBEdit18: TdxDBEdit
      Left = 446
      Top = 295
      Width = 154
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 23
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      OnKeyDown = DbCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'ALTURA_PISTAO'
      DataSource = DataSource
    end
    object edObservacoes: TdxDBMemo
      Left = 328
      Top = 394
      Width = 273
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 26
      OnEnter = EdInicialEnter
      OnExit = EdInicialExit
      DataField = 'OBSERVACAO'
      DataSource = DataSource
      Height = 74
    end
    object EdMontadorVirabrequim: TdxDBEdit
      Left = 126
      Top = 176
      Width = 34
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 5
      OnEnter = EdInicialEnter
      OnExit = EdMontadorVirabrequimExit
      OnKeyDown = DbCodigoKeyDown
      DataField = 'MONTADOR_VIRABREQUIM'
      DataSource = DataSource
    end
    object dxDBEdit19: TdxDBEdit
      Left = 160
      Top = 176
      Width = 121
      Color = clMenu
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      Style.HotTrack = True
      TabOrder = 28
      TabStop = False
      DataField = 'NOME_MONTADOR_VIRABREQUIM'
      DataSource = DataSource
      ReadOnly = True
      StoredValues = 64
    end
    object EdMontadorCabecote: TdxDBEdit
      Left = 126
      Top = 320
      Width = 34
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 10
      OnEnter = EdInicialEnter
      OnExit = EdMontadorCabecoteExit
      OnKeyDown = DbCodigoKeyDown
      DataField = 'MONTADOR_CABECOTE'
      DataSource = DataSource
    end
    object dxDBEdit21: TdxDBEdit
      Left = 160
      Top = 320
      Width = 121
      Color = clMenu
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      Style.HotTrack = True
      TabOrder = 29
      TabStop = False
      DataField = 'NOME_MONTADOR_CABECOTE'
      DataSource = DataSource
      ReadOnly = True
      StoredValues = 64
    end
    object EdMontadorBiela: TdxDBEdit
      Left = 446
      Top = 176
      Width = 34
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 19
      OnEnter = EdInicialEnter
      OnExit = EdMontadorBielaExit
      OnKeyDown = DbCodigoKeyDown
      DataField = 'MONTADOR_BIELA'
      DataSource = DataSource
    end
    object dxDBEdit22: TdxDBEdit
      Left = 481
      Top = 176
      Width = 119
      Color = clMenu
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      Style.HotTrack = True
      TabOrder = 30
      TabStop = False
      DataField = 'NOME_MONTADOR_BIELA'
      DataSource = DataSource
      ReadOnly = True
      StoredValues = 64
    end
    object EdMontadorAjuste: TdxDBEdit
      Left = 446
      Top = 320
      Width = 34
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 24
      OnEnter = EdInicialEnter
      OnExit = EdMontadorAjusteExit
      OnKeyDown = DbCodigoKeyDown
      DataField = 'MONTADOR_AJUSTE'
      DataSource = DataSource
    end
    object dxDBEdit23: TdxDBEdit
      Left = 480
      Top = 320
      Width = 121
      Color = clMenu
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      Style.HotTrack = True
      TabOrder = 31
      TabStop = False
      DataField = 'NOME_MONTADOR_AJUSTE'
      DataSource = DataSource
      ReadOnly = True
      StoredValues = 64
    end
    object EdMontadorBloco: TdxDBEdit
      Left = 126
      Top = 444
      Width = 34
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 14
      OnEnter = EdInicialEnter
      OnExit = EdMontadorBlocoExit
      OnKeyDown = DbCodigoKeyDown
      DataField = 'MONTADOR_BLOCO'
      DataSource = DataSource
    end
    object dxDBEdit24: TdxDBEdit
      Left = 160
      Top = 444
      Width = 121
      Color = clMenu
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      Style.HotTrack = True
      TabOrder = 32
      TabStop = False
      DataField = 'NOME_MONTADOR_BLOCO'
      DataSource = DataSource
      ReadOnly = True
      StoredValues = 64
    end
    object EdMontador: TdxDBEdit
      Left = 445
      Top = 352
      Width = 34
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      TabOrder = 25
      OnEnter = EdInicialEnter
      OnExit = EdMontadorExit
      OnKeyDown = DbCodigoKeyDown
      DataField = 'MONTADOR'
      DataSource = DataSource
    end
    object dxDBEdit25: TdxDBEdit
      Left = 479
      Top = 352
      Width = 121
      Color = clMenu
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.ButtonStyle = btsSimple
      Style.HotTrack = True
      TabOrder = 33
      TabStop = False
      DataField = 'NOME_MONTADOR'
      DataSource = DataSource
      ReadOnly = True
      StoredValues = 64
    end
  end
  object BarDBNavigator: TdxBarDBNavigator
    BarManager = BarManager
    CategoryName = 'DB Navigator'
    ConfirmDelete = False
    DataSource = DataSource
    DBCheckLinks = <>
    VisibleButtons = [dxbnFirst, dxbnPrior, dxbnNext, dxbnLast]
    Left = 645
    Top = 127
  end
  object BarManager: TdxBarManager
    AllowReset = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    CanCustomize = False
    Categories.Strings = (
      'Default'
      'DB Navigator')
    Categories.ItemsVisibles = (
      2
      2)
    Categories.Visibles = (
      True
      True)
    PopupMenuLinks = <>
    Style = bmsFlat
    UseSystemFont = True
    Left = 645
    Top = 32
    DockControlHeights = (
      0
      0
      0
      0)
    object BarManagerBar1: TdxBar
      AllowClose = False
      AllowCustomizing = False
      AllowQuickCustomizing = False
      AllowReset = False
      Caption = 'Navega'#231#227'o'
      CaptionButtons = <>
      DockControl = dxBarDockControl1
      DockedDockControl = dxBarDockControl1
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 290
      FloatTop = 163
      FloatClientWidth = 23
      FloatClientHeight = 198
      ItemLinks = <
        item
          Visible = True
          ItemName = 'dxBarBDBNavFirst'
        end
        item
          Visible = True
          ItemName = 'dxBarBDBNavPrev'
        end
        item
          Visible = True
          ItemName = 'dxBarBDBNavNext'
        end
        item
          Visible = True
          ItemName = 'dxBarBDBNavLast'
        end
        item
          Visible = True
          ItemName = 'btnIncluir'
        end
        item
          Visible = True
          ItemName = 'btnExcluir'
        end
        item
          Visible = True
          ItemName = 'OptAlterar'
        end
        item
          Visible = True
          ItemName = 'btnLocalizar'
        end
        item
          Visible = True
          ItemName = 'BtnListagem'
        end
        item
          Visible = True
          ItemName = 'BtnSalvar'
        end
        item
          Visible = True
          ItemName = 'BtnCancelar'
        end
        item
          Visible = True
          ItemName = 'BtnFechar'
        end>
      OldName = 'Navega'#231#227'o'
      OneOnRow = True
      Row = 0
      ShowMark = False
      SizeGrip = False
      UseOwnFont = False
      UseRestSpace = True
      Visible = True
      WholeRow = False
    end
    object btnIncluir: TdxBarButton
      Action = ActIncluir
      Category = 0
      Glyph.Data = {
        
        888888800000000888888888888888888888}
    end
    object btnExcluir: TdxBarButton
      Action = ActExcluir
      Category = 0
      Glyph.Data = {
        
        0000000000000000008080008080008080008080008080000000}
    end
    object btnLocalizar: TdxBarButton
      Action = ActLocalizar
      Category = 0
      Glyph.Data = {
        
        D7D7D7D7AC56ACAC0000ACAC565656565656565656ACACAC0000}
    end
    object BtnSalvar: TdxBarButton
      Action = ActPost
      Category = 0
      Enabled = False
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBF}
    end
    object BtnCancelar: TdxBarButton
      Action = ActCancel
      Category = 0
      Enabled = False
      Glyph.Data = {
        
        8000808000808000808000808000808000808000808000808000}
    end
    object BtnListagem: TdxBarButton
      Action = ActListagem
      Category = 0
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBF0000}
    end
    object BtnFechar: TdxBarButton
      Action = ActFechar
      Align = iaRight
      Category = 0
      Glyph.Data = {
        
        0000000000000000000000000000000000008080008080000000}
      PaintStyle = psCaptionGlyph
    end
    object OptAlterar: TdxBarButton
      Action = ActAlterar
      Category = 0
      Glyph.Data = {
        
        0000000000000000000000000000000000008080008080008080}
    end
    object dxBarBDBNavFirst: TdxBarDBNavButton
      Caption = 'ActInicio'
      Category = 1
      Enabled = False
      Hint = 'Ir ao primeiro registro'
      Visible = ivAlways
      Glyph.Data = {
        
        7777777777777777777777777777777777777777777777777777}
      NavButton = dxbnFirst
    end
    object dxBarBDBNavPrev: TdxBarDBNavButton
      Caption = 'ActPrior'
      Category = 1
      Enabled = False
      Hint = 'Ir ao registro anterior'
      Visible = ivAlways
      Glyph.Data = {
        
        7777777777777777777777777777777777777777777777777777}
      NavButton = dxbnPrior
    end
    object dxBarBDBNavNext: TdxBarDBNavButton
      Caption = 'ActNext'
      Category = 1
      Enabled = False
      Hint = 'Ir ao pr'#243'ximo registro'
      Visible = ivAlways
      Glyph.Data = {
        
        7777777777777777777777777777777777777777777777777777}
      NavButton = dxbnNext
    end
    object dxBarBDBNavLast: TdxBarDBNavButton
      Caption = 'ActLast'
      Category = 1
      Enabled = False
      Hint = 'Ir ao '#250'ltimo registro'
      Visible = ivAlways
      Glyph.Data = {
        
        7777777777777777777777777777777777777777777777777777}
      NavButton = dxbnLast
    end
  end
  object Actions: TActionList
    Left = 329
    Top = 48
    object ActIncluir: TAction
      Tag = 1
      Caption = 'ActIncluir'
      Hint = 'Incluir um novo registro [Ins]'
      ShortCut = 45
      OnExecute = ActIncluirExecute
    end
    object ActAlterar: TAction
      Tag = 1
      Caption = 'ActAlterar'
      ShortCut = 117
      OnExecute = ActAlterarExecute
    end
    object ActExcluir: TAction
      Tag = 1
      Caption = 'ActExcluir'
      Hint = 'Excluir o registro selecionado [Shift+Del]'
      ShortCut = 8238
      OnExecute = ActExcluirExecute
    end
    object ActLocalizar: TAction
      Caption = 'ActLocalizar'
      Hint = 'Localizar registros [F3]'
      ShortCut = 114
      OnExecute = ActLocalizarExecute
    end
    object ActListagem: TAction
      Tag = 1
      Caption = 'ActListagem'
      Hint = 'Emitir listagem de '
      ShortCut = 116
      OnExecute = ActListagemExecute
    end
    object ActPost: TAction
      Caption = 'ActPost'
      Hint = 'Salvar informa'#231#245'es'
      ShortCut = 121
      OnExecute = ActPostExecute
    end
    object ActCancel: TAction
      Caption = 'ActCancel'
      Hint = 'Cancelar informa'#231#245'es'
      ShortCut = 27
      OnExecute = ActCancelExecute
    end
    object ActFechar: TAction
      Caption = '&Fechar  '
      OnExecute = ActFecharExecute
    end
    object ActLocalizarMontador: TAction
      Caption = 'Localizar Montador'
      OnExecute = ActLocalizarMontadorExecute
    end
  end
  object DataSource: TDataSource
    AutoEdit = False
    DataSet = DmServicos.Ctrl_Qualidade
    OnStateChange = DataSourceStateChange
    Left = 645
    Top = 184
  end
  object DsMecanicos: TDataSource
    AutoEdit = False
    DataSet = DmServicos.SelMecanico
    Left = 645
    Top = 232
  end
end
