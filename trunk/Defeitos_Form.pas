...
begin
  ...
  If FrmDefeitos.FormStyle = fsMDIChild Then
...
procedure TFrmDefeitos.ActIncluirExecute(Sender: TObject);
begin
...
procedure TFrmDefeitos.ActExcluirExecute(Sender: TObject);
begin
  If MessageBox(Handle,'Tem certeza que deseja Excluir este Defeito ?',
                       'Atenção !!!',MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2)=IDYES Then
     DataSource.Dataset.Delete;
end;

procedure TFrmDefeitos.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmDefeitos.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmDefeitos.FormShow(Sender: TObject);
begin
...
procedure TFrmDefeitos.ActListagemExecute(Sender: TObject);
begin
  //
...
procedure TFrmDefeitos.ActLocalizarExecute(Sender: TObject);
begin
  try
        ...
        Frm_Localizar_Generico.LblTitulo.Caption := 'Localizar Defeito' ;
...
procedure TFrmDefeitos.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmDefeitos.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmDefeitos.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmDefeitos.ActFecharExecute(Sender: TObject);
begin
  If FrmDefeitos.FormStyle = fsMDIChild Then
...
procedure TFrmDefeitos.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount);
end;

procedure TFrmDefeitos.ActAlterarExecute(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Exit;
  Try
...
procedure TFrmDefeitos.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmDefeitos.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
