...
uses Application_DM,
     ...
     Procedures4_DM,
     Funcoes_Daruma,
     UDS300, Procedures5_DM, Vendas_Dm, Cartao_Venda_Form,
     ...
     THEN BEGIN
          ...
          THEN BEGIN
               ...
               THEN BEGIN
                    ...
                    Daruma_DUAL_ImprimirTexto('<b>'+aux+'</b> ' , 0);

//                    Daruma_DUAL_ImprimirTextoFormatado ( Aux,  1, 0, 0, 0, 0, 0);

                    SLEEP ( 10 );

                    Aux := StrDs300 (COPY(DmApp.Nome,1,48),  ' ');

                    Daruma_DUAL_ImprimirTextoFormatado ( Aux,  1, 0, 0, 0, 0, 0);

                    SLEEP ( 10 );

                    Aux := StrDs300 (COPY(DmApp.ENDER + ', BAIRRO = ' + DMAPP.BAIRRO,1,40),  ' ');

                    Daruma_DUAL_ImprimirTexto(Aux, 0);

                    SLEEP ( 10 );

                    Aux := StrDs300 (COPY(DmApp.CIDADE,1,40),  ' ');

                    Daruma_DUAL_ImprimirTexto( Aux, 0);

                    SLEEP ( 10 );

                    Aux := StrDs300 (COPY('FONE ' + DmApp.FONE + ' FAX ' + DMAPP.FAX,1,40),  ' ');
               ...
               Aux := StrDs300 ('RECIBO N => ' + STR,  ' ');

               Daruma_DUAL_ImprimirTexto(Aux, 0);

               SLEEP ( 10 );

               STR   := COPY('RECIBI(EMOS) DE  ' + FAVORECIDO,1 ,48) ;
               STR2  := COPY('RECIBI(EMOS) DE  ' + FAVORECIDO,49,48) ;

               Aux := StrDs300 (STR,  ' ');

               Daruma_DUAL_ImprimirTexto(STR, 0);
               ...
               STR1 := COPY('A IMPORTANCIA DE R$ ' + STR + ' ( ' + EXTENSO.Texto + ' ) ',01,48) ;
               STR2 := COPY('A IMPORTANCIA DE R$ ' + STR + ' ( ' + EXTENSO.Texto + ' ) ',49,48) ;
               STR3 := COPY('A IMPORTANCIA DE R$ ' + STR + ' ( ' + EXTENSO.Texto + ' ) ',97,48) ;
               Aux := StrDs300 (STR1,  ' ');
               ...
               STR  := COPY ('PROVENIENTE DE : ' + DOCTO + '  ' + HISTORICO,01,48);
               STR1 := COPY ('PROVENIENTE DE : ' + DOCTO + '  ' + HISTORICO,49,48);
               STR2 := COPY ('PROVENIENTE DE : ' + DOCTO + '  ' + HISTORICO,96,48);
               ...
               STR  := COPY(DMAPP.CIDADE + ', ' + INTTOSTR(DIA) + ' DE ' + MESEXT + ' DE ' + INTTOSTR(ANO),01,48);
               STR1 := COPY(DMAPP.CIDADE + ', ' + INTTOSTR(DIA) + ' DE ' + MESEXT + ' DE ' + INTTOSTR(ANO),48,48);
               STR2 := COPY(DMAPP.CIDADE + ', ' + INTTOSTR(DIA) + ' DE ' + MESEXT + ' DE ' + INTTOSTR(ANO),96,48);
...
//                  SLEEP ( 10 );
               ...
               IF (( DmApp.Perfil_ComputadorAUTENTICAR.Value = 'S' ) and ( PERGUNTA( 'Autenticar Documento?' ) = 'S' ))
               THEN BEGIN
                    Autenticar ( DmApp.Nome + ' ' + NUMRECIBO);
               END;
          END
          ELSE BEGIN
               DECODEDATE ( DT_LANCTO, ANO, MES, DIA );
...
begin
   ...
   begin
     ...
     Showmessage('Posicione o Documento!');
     ...
     then begin
          ...
          begin
               ...
               Str_Texto_Autenticacao := '<sn><c>' + Texto + '</c> (D:<dt></dt> H:<hr></hr>)</sn>';
              ...
              If Int_Retorno = -50 Then
                 Showmessage('-50 - Impressora OFF-LINE", vbCritical, "Daruma Framework')
              else
                  If Int_Retorno = -51 Then
                     Showmessage('-51 - Impressora Sem Papel", vbCritical, "Daruma Framework')
                  else
                      If Int_Retorno = -27 Then
                         Showmessage('-27 - Erro Genérico", vbCritical, "Daruma Framework')
                      else
                          If Int_Retorno = 0 Then
                             Showmessage('0 - Impressora Desligada", vbCritical, "Daruma Framework');
                          end
                          else begin
                               Showmessage('Erro na Configuração do Registro para Autenticação!');
                          end;
     ...
     begin
      Application.MessageBox(' Erro!', 'Erro ao Configurar a Porta da Impressora, Verifique!', mb_ok + 16);
     end;
end;
